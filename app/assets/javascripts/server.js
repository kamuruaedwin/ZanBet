...
// Route for rendering the animated chart
app.get('/render-animation', (req, res) => {
  // Execute your animation logic here (similar to the client-side script)
  // ... <script>
        document.addEventListener("DOMContentLoaded", () => {
      const canvas = document.getElementById('chart-container');
      const context = canvas.getContext('2d');
      const animationDuration = 28000; // Animation duration in milliseconds
      let animationStartTime;
      let burst_value;
      const container = document.getElementById('container');

      let animationIsRunning = true; // Initially, the animation is running
      let randomStopTime = Math.random() * 28000; // Random stopping point (approximately 30 seconds);
      ...
      function stopAnimation(value) {
        ...
        const countdownInterval = setInterval(() => {
          ...
          if (countdown === 0) {
            ...
            randomStopTime = Math.random() * 28000;
            startAnimation();
  ...
  const generatedHTML = `
    ...
    <body>
      <!-- The following script should be included in the generated HTML -->
